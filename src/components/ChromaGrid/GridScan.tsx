...
export const GridScan: React.FC<GridScanProps> = ({
  ...
  return (
    <div
      ref={containerRef}
      className={`gridscan w-full h-screen ${className || ""}`}
      style={style}
    >
      {showPreview && (
        <div className="gridscan__preview relative w-full h-full">
          <video
            ref={videoRef}
            muted
            playsInline
            autoPlay
            className="gridscan__video w-full h-full object-cover"
          />
          <div className="gridscan__badge absolute top-2 left-2 bg-black/50 text-white px-2 py-1 rounded">
            {enableWebcam
              ? modelsReady
                ? uiFaceActive
                  ? "Face: tracking"
